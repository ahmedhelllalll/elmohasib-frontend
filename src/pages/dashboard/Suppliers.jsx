...
const SuppliersComponent = () => {
    const { suppliers, isLoading, addSupplier, updateSupplier, deleteSupplier } = useContext(SupplierContext);
    ...
    useEffect(() => {
        if (!isLoading && suppliers.length > 0 && listRef.current) {
        ...
        <div className="h-full flex flex-col space-y-4 md:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">قم بإدارة مورديك، وتتبع بياناتهم ومديونياتهم لتسهيل عمليات المشتريات.</p>
                </div>
                <button 
                    onClick={handleAdd}
            ...
            {!isLoading && filteredSuppliers.length === 0 && (
                <div className="flex-1 flex flex-col items-center justify-center bg-white rounded-2xl border border-slate-100 border-dashed p-8 text-center min-h-[400px]">
                    ...
                    <h3 className="text-lg font-bold text-slate-800">لا يوجد موردين</h3>
                    <p className="text-slate-500 text-sm mt-1 mb-6 max-w-sm">
                        لم يتم العثور على أي موردين. يمكنك إضافة مورد جديد للبدء في تتبع عمليات الشراء الخاصة بهم.
                    </p>
                    <button 
                        onClick={handleAdd}
                ...
                {isLoading ? (
                    ...
                    <table className="w-full text-sm text-right text-slate-500 hidden md:table">
                        <thead className="text-xs text-slate-700 bg-slate-50/50 uppercase border-b border-slate-100">
                            <tr>
                                <th className="px-6 py-4 rounded-ts-2xl">اسم المورد</th>
                            ...
                            {filteredSuppliers.map((Supplier) => (
                                <tr key={Supplier.id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 font-medium text-slate-700">
                                        {Supplier.company_name || <span className="text-slate-400 font-normal">لا يوجد</span>}
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="font-bold text-rose-600">
                                            {Supplier.credit_balance > 0 ? parseFloat(Supplier.credit_balance).toLocaleString('en-US') + ' ر.س' : '0'}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button onClick={() => handleDelete(Supplier.id)} className="p-2 text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="حذف">
            ...
            <div className="md:hidden space-y-3 pb-20">
                {!isLoading && filteredSuppliers.map((Supplier) => (
                    <div key={Supplier.id} className="bg-white p-4 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.02)] border border-slate-100">
                        <div className="flex justify-between items-start mb-3">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-slate-800 text-base leading-tight">{Supplier.name}</h3>
                                    {Supplier.company_name && <p className="text-xs text-slate-500 mt-0.5">{Supplier.company_name}</p>}
                                </div>
                            </div>
                            <div className="text-left">
                                <span className="text-[10px] text-slate-400 block mb-0.5">المديونية</span>
                                <span className="font-bold text-rose-600 text-sm block">
                                    {Supplier.credit_balance > 0 ? parseFloat(Supplier.credit_balance).toLocaleString('en-US') : '0'}
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-end mt-4 pt-3 border-t border-slate-50 gap-2">
                            <button onClick={() => handleEdit(Supplier)} className="p-2 text-blue-600 bg-blue-50 rounded-lg flex-1 flex justify-center items-center gap-2 font-bold text-sm">
                                <Edit2 className="w-3.5 h-3.5" /> تعديل
                            </button>
                            <button onClick={() => handleDelete(Supplier.id)} className="p-2 text-rose-600 bg-rose-50 rounded-lg flex-1 flex justify-center items-center gap-2 font-bold text-sm">
